...
class PositionEmbeddingCoordsSine(nn.Module):
    ...
    def get_sine_embeddings(self, xyz, num_channels):
        ...
        ndim = num_channels // xyz.shape[-1]
        if ndim % 2 != 0:
            ndim -= 1
        # automatically handle remainder by assiging it to the first dim
        rems = num_channels - (ndim * xyz.shape[-1])
    ...
    def get_fourier_embeddings(self, xyz, num_channels=None):
        ...
        # return batch * d_pos x npoints embedding
        final_embeds = torch.cat(final_embeds, dim=1)
        return final_embeds
